...
export default function OrdersList() {
  ...
  useEffect(() => {
    ...
    // Load Firebase data only once on mount
    const timeoutId = setTimeout(loadFirebaseData, 500);
  ...
  const handleDelete = async (orderId: string) => {
    ...
    console.log('🔥 ID para excluir:', orderId);
    console.log('🔥 serviceOrders antes:', serviceOrders.length);
    console.log('🔥 Lista atual:', serviceOrders.map(o => ({ id: o.id, tecnico: o.tecnico })));
    
    if (!confirm(`Excluir ordem ${orderId}?`)) {
    ...
    try {
      ...
      console.log('✅ Excluído do Firebase com sucesso');
      
      // Then update local state
      console.log('🔥 Confirmado - filtrando ordens...');
      const ordensAntes = [...serviceOrders];
      const novasOrdens = serviceOrders.filter(order => {
        const manter = order.id !== orderId;
        console.log(`🔥 Ordem ${order.id}: ${manter ? 'MANTER' : 'REMOVER'}`);
        return manter;
      });
      
      console.log('🔥 Ordens antes do filtro:', ordensAntes.length);
      console.log('🔥 Ordens após filtro:', novasOrdens.length);
      console.log('🔥 Diferença:', ordensAntes.length - novasOrdens.length);
      
      console.log('🔥 Chamando setServiceOrders...');
      setServiceOrders(novasOrdens);
      console.log('🔥 setServiceOrders chamado');
      
      alert('Ordem removida!');
      console.log('🎉 Exclusão completa - Firebase e local sincronizados');
      
    } catch (error) {
      console.error('❌ Erro ao excluir do Firebase:', error);
      // Still update local state even if Firebase fails
      const novasOrdens = serviceOrders.filter(order => order.id !== orderId);
      setServiceOrders(novasOrdens);
      console.log('⚠️ Excluído apenas localmente devido ao erro no Firebase');
      alert('Ordem removida localmente (erro no Firebase)');
    }
    
    console.log('🔥 ===== FIM EXCLUSÃO =====');
  };
